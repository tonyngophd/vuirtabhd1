...
import com.suas.vuirtab1.IVedeoInterface.Stub;
...
public class VideoServiceImpl extends Stub implements JniBridge.networkInteface {
    ...
    public int onData(int idx, int len, long timestamp, int flag) {
        ...
        try {
            ...
            Log.e(this.TAG, "onData got exception , but ignored " + e.toString());
        }
        return 0;
    }
                ...
                if (this.mContinue) {
                    switch (outIndex) {
                        ...
                        default:
                            ...
                            //this.outputBuffers = this.mMediaCodec.getOutputBuffer(outIndex);
                            /*Image image = this.mMediaCodec.getOutputImage(outIndex);
                            assert image != null;
                            this.outputBuffers = image.getPlanes()[0].getBuffer();*/
                            /*this.outputBuffers = this.mMediaCodec.getOutputBuffer(0);
                            byte[] bytes = new byte[this.outputBuffers.capacity()];
                            this.outputBuffers.get(bytes);
                            ControlsFragment.bitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.length, null);
                            //Log.i(TAG, "renderthread: got a bitmap");*/
                            if (this.mCodecStatus != 1) {
