...
import static com.suas.vuirtab1.MediaGalleryActivity.TAG;
...
public class MediaGalleryFragment extends Fragment {
        ...
        sharebutton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (createLists.get(index).getImage_Location().contains("jpg")) {
                    ...
                    type = "video/mp4";
                } else if (createLists.get(index).getImage_Location().contains("mjpeg")) {
                    type = "video/mjpeg";
                } else if (createLists.get(index).getImage_Location().contains("mov")) {
                    type = "video/mov";
                }

                File imagefile = new File(createLists.get(index).getImage_Location());
                Log.i(TAG, "onLongPress: file = " + createLists.get(index).
    ...
    private ArrayList<CreateList> prepareData(String folder) {
        ...
        Log.d(TAG, "prepareData: path = " + path);
        if (folder.length() > 11) {
            path = getExternalStorageDirectories()[0] + "/" + folder;
        }
        //Log.i(TAG, "prepareData: sdCard path = " + sdCardPath());
        ...
        if (file != null) {
            ...
            for (int i = 0; i < file.length; i++) {
                if (file[i].getName().contains("jpg") || file[i].getName().contains("mp4")
                        || file[i].getName().contains("mjpeg")
                        || file[i].getName().contains("mov")) {
                    ...
                    if (file_size_in_kB < 300 && (file[i].getName().contains("mp4") || file[i].getName().contains("mjpeg") || file[i].getName().contains("mov")))
                        continue; //Discard corrupted or too small (< 10kB) files so it won't crash the app
                    maxFileNumbers++;
    ...
    private ArrayList<CreateList> prepareDataFolder() {
        ...
        Log.d(TAG, "prepareData: path = " + path);
        File f = new File(path);
        String[] sdCardPaths = getExternalStorageDirectories();
        int n3 = 0;
        File fflir = new File("");
        if (sdCardPaths.length > 0) {
            String sdCardPath = getExternalStorageDirectories()[0];
            fflir = new File(sdCardPath);
            Log.d(TAG, "prepareDataFolder: getExternalStorageDirectories[0] = " + getExternalStorageDirectories()[0] + " length = " + getExternalStorageDirectories().length);
            if(fflir.listFiles() != null) {
                n3 = Objects.requireNonNull(fflir.listFiles()).length;
            }
        }
        int n1 = 0;
        if(f.listFiles() != null) {
            n1 = Objects.requireNonNull(f.listFiles()).length;
        }
        int n2 = 0;
        for (int i = 0; i < n3; i++) {
            String nameraw = Objects.requireNonNull(fflir.listFiles())[i].toString();
            String name = nameraw.substring(nameraw.lastIndexOf('/') + 1);
            Log.i(TAG, "prepareDataFolder: raw folder = " + name);
            if (name.length() > 14 && name.substring(0, 2).contains("20")) {
                Log.d(TAG, "prepareDataFolder: flir folder = " + name);
                n2++;
            }
        }
        File[] files = new File[n1 + n2];
        for (int i = 0; i < n1; i++) {
            files[i] = Objects.requireNonNull(f.listFiles())[i];
        }
        for (int i = n1, j = n1; i < n1 + n3; i++) {
            String nameraw = Objects.requireNonNull(fflir.listFiles())[i - n1].toString();
            String name = nameraw.substring(nameraw.lastIndexOf('/') + 1);
            if (name.length() > 14 && name.substring(0, 2).contains("20")) {
                Log.d(TAG, "prepareDataFolder: flir folder = " + name);
                files[j] = Objects.requireNonNull(fflir.listFiles())[i - n1];
                j++;
            }
        }
    ...
    private String[] getExternalStorageDirectories() {
        ...
        File[] externalDirs = Objects.requireNonNull(getActivity()).getExternalFilesDirs(null);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            for (int i = 0; i < results.size(); i++) {
                if (!results.get(i).toLowerCase().matches(".*[0-9a-f]{4}[-][0-9a-f]{4}")) {
                    Log.d("Media", results.get(i) + " might not be extSDcard");
                    results.remove(i--);
                }
            }
        } else {
